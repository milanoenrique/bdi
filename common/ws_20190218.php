<?php 
    ...
    {
         //Ordered_delay
        $sth = $dbh->prepare("update requests set reqstatus = 'ORD' , idpriority = 'D' from (select dead_line from requests, request_order_deadline where dead_line < current_date and requestid = ".$row['idrequest']." and reqstatus ='OR' ) as subquery where idrequest = ".$row['idrequest']."");
        $sth->execute();
        //Ordered_delay
        $dashboard[] = array
        (
