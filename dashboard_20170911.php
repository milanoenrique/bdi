<?php
	...
	if(!isset($_SESSION['getValidateUser']) || empty($_SESSION['getValidateUser'])) {
		header("Location: http://reqparts.bdicalgary.com/BrandellDieselIS/index_tech.php");
		die();
		}
	}
...
<html>
                                        ...
                                        <center>
                                            <div class="panel-body" id="partsRequesition">
                                                <form class="form-horizontal" style="text-align: left;" data-toggle="validator">
                                                    ...
                                                    <div class="form-group">
                                                        <label class="col-md-3 control-label" for="techName"></label>
                                                        <div class="col-md-9  inputGroupContainer" style="padding-right: 0;">
                                                            <input id="priority" type="checkbox" name="priority"> 911
                                                        </div>
                                                    </div>
                                                    <div class="form-group">
                                                        <label class="col-md-3 control-label" for="techName"></label>
                                                        <div class="col-md-9  inputGroupContainer" style="padding-right: 0;">
                                                            <input id="requestType" type="checkbox" name="requestType"> Quote
                                                        </div>
                                                    </div>
                                                    <div class="form-group">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">					
						...
						<button type="button" class="btn btn-primary" data-dismiss="modal" style="background:#2C3E50;border-color:#2C3E50;" id="print" onclick="printDivReq('modal-view')">Print</button>
						<button type="button" class="btn btn-primary" data-dismiss="modal" style="background:#2C3E50;border-color:#2C3E50;">Close</button>
                    </div>                  
                </div>
            </div>
	...
	<script src="js/jquery-1.11.2.min.js"></script>
		...
		{	console.log('USUARIO VACIO');
			window.location="/BrandellDieselID/index_tech.php";}
...
// Copy from edit							
                                ...
                                call_Ajax_Jsonp(URL, METODO, VALOR, LOADER, ERROR, function(data) 
                                {
                                    ...
                                    if(DATA.REQUESTS[0].idpriority === "H"){
                                        $("#modal-view #priority").prop('checked', true);} 
                                    else {
                                        $("#modal-view #priority").prop('checked', false);}
									$("#modal-view #priority").prop('disabled',true);
                                    if(DATA.REQUESTS[0].idrequesttype === "Q"){
                                        $("#modal-view #requestType").prop('checked', true);} 
                                    else {
                                        $("#modal-view #requestType").prop('checked', false);}
									$("#modal-view #requestType").prop('disabled',true);
...
// End copy from edit							
                            ...
                            { 
                                ...
                                call_Ajax_Jsonp(URL, METODO, VALOR, LOADER, ERROR, function(data) 
                                {
                                    ...
                                    //console.log(parts);
                                    if(DATA.REQUESTS[0].idpriority === "H"){
                                        $("#modal-edit #priority").prop('checked', true);} 
                                    else {
                                        $("#modal-edit #priority").prop('checked', false);}
					...
					//$('#table-dashboard').bootstrapTable('destroy');
					//alert($('input:text').val('CARLOS ES UN PIPIRINICE..!'));
...
$('input:radio[name="optradio"]').change(function(){
	...
	else
	{
		...
		$("#myModalSendMessage #buttonSendMessage").click(function()
				...
				window.location.replace("http://reqparts.bdicalgary.com/BrandellDieselID/index.php");
				}

						$(".modal-body #priority").removeAttr('checked');
				...
				$("#modal-edit #buttonSave").click(function()
                {
					...
					if($("#modal-edit #requestType").is(':checked')){
                        idrequesttype   = "Q";
						vType = "Quote";
                    } else {
                        idrequesttype   = "O";
                    ...
                    if($("#modal-edit #priority").is(':checked')){
                        idpriority   = "H";
						vPriority = "911";
                     } else {
                        idpriority   = "N";
                    ...
                    partsJSON   = JSON.stringify(parts);
					rederizsoftReady('A','#partsRequesition', 'common/sendMail.php?vRo='+encodeURIComponent(ro)+'&vtechName='+encodeURIComponent(techName)+'&vPriority='+encodeURIComponent(vPriority)+'&vType='+encodeURIComponent(vType)+'&vVin='+encodeURIComponent(vin)+'&vComments='+encodeURIComponent(comments)+'&vpartsJSON='+encodeURIComponent(partsJSON)+'&vDate='+encodeURIComponent(moment(DATE).format("MMM DD YYYY, hh:mm:ss a"))+'&vEngine='+encodeURIComponent(engine)+'&vStatus=', '#cargar_data_edit', 'consultar');
		...
		$( "#adminUsers" ).click(function() 
                {
					...
					call_Ajax_Jsonp(URL, METODO, VALOR, LOADER, ERROR, function(data) 
					{
						...
						optionsGroups.append('<option value="" selected>--None--</option>');
						console.log('<option value="" selected>--None--</option>');
					...
					/* se quito porq se perdia el filtro del dashboard
					appuser = $(".modal-body #techId").val();
					VALOR   = '';
					<?php 
					if ($profile=='TECH') {
                ...
                $("#modal-adminUsers #buttonAddUser").click(function()
                {
					...
					// 2017-09-05 MULTIPLE GROUPS ADD
					console.log(idgroup);
					var glist = document.getElementsByTagName('select')[0];
					var groups = getSelectValues(glist);
					console.log(groups);
		    ...
		    {	
                ...
                $("#modal-adminUsers #buttonEditUser").click(function()
                {
                    ...
                    METODO      = "";
                    VALOR       = iduser + "|" + surname + "|" + lastname + "|" + password + "|" + idusertype + "|" + appuser + "|" + idrol + "|" + phonenum + "|" + group;
			...
			function reloadDashboard(paramVALOR, paramUrl) {
				...
				if ( iduser  === null||iduser === "") {
					//console.log('Your session expire');
					window.location.replace("http://reqparts.bdicalgary.com/BrandellDieselIS/index.php");
				}
				else
				{ 
			...
			function sendPDF () 
			       {
					...
					alert ('Voy al PDF');
					console.log('common/sendMail.php?&vRo='+encodeURIComponent(ro)+'&vtechName='+encodeURIComponent(techName)+'&vPriority='+encodeURIComponent(vPriority)+'&vType='+encodeURIComponent(vType)+'&vVin='+encodeURIComponent(vin)+'&vComments='+encodeURIComponent(comments)+'&vpartsJSON='+encodeURIComponent(partsJSON)+'&vDate='+requestdate+'&vEngine='+encodeURIComponent(engine)+'&vStatus=');
					rederizsoftReady('A','#partsRequesition', 'common/sendMail.php?vRo='+encodeURIComponent(ro)+'&vtechName='+encodeURIComponent(techName)+'&vPriority='+encodeURIComponent(vPriority)+'&vType='+encodeURIComponent(vType)+'&vVin='+encodeURIComponent(vin)+'&vComments='+encodeURIComponent(comments)+'&vpartsJSON='+encodeURIComponent(partsJSON)+'&vDate='+encodeURIComponent(requestdate)+'&vEngine='+encodeURIComponent(engine)+'&vStatus=', '#cargar_data_edit', 'consultar');
					//generatePDFreq(idrequest,ro, appuser, moment(DATE).format("MMM DD YYYY, h:mm:ss a") ,comments, vPriority, vType, vin, parts);
                    METODO          = partsJSON;
                    VALOR           = idrequest + "|" + jobnumber + "|" + appuser + "|" + idrequesttype + "|" + idpriority + "|" + ro + "|" + vin + "|" + trans + "|" + engine + "|" + comments;
                    call_Ajax_Jsonp(URL, METODO, VALOR, LOADER, ERROR, function(data) 
                    {
